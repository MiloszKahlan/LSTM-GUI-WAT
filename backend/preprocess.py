...
KERN_DATASET_PATH = "data/dataset/"
...
def preprocess(dataset_path):
    ...
    print(f"{len(songs)} song(s) processed")
...
def generate_training_sequences(sequence_length, test_size=0.2):
    """Create input and output data samples for training and validation. Each sample is a sequence.

    :param sequence_length (int): Length of each sequence. With a quantisation at 16th notes, 64 notes equates to 4 bars
    :param test_size (float): Fraction of data to be used as validation set

    :return x_train (ndarray): Training inputs
    :return x_val (ndarray): Validation inputs
    :return y_train (ndarray): Training targets
    :return y_val (ndarray): Validation targets
    """
    ...
    x_train, x_val, y_train, y_val = train_test_split(inputs, targets, test_size=test_size, random_state=42)

    print(f"There are {len(x_train)} training sequences and {len(x_val)} validation sequences.")
